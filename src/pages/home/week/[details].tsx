...
const Index = (courses: any, week: any) => {
...
export async function getStaticPaths() {
    ...
    const paths = courses.courseWeeks.map((week: any) => ({
        params: {
...
export const getStaticProps: GetStaticProps = async (context:any ) => {
    const res = await fetch("https://jsonkeeper.com/b/M6QQ");
    const courses: Course = await res.json();
    const week = courses?.courseWeeks?.find((item: any) => parseInt(item.id) === parseInt(context.params?.details));
    return {
        props: {
            courses,
            week,
